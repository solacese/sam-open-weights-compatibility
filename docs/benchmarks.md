...
The 22 models above are the ones SAM can drive. Three more are documented but **excluded from the ranking** because they are `unsupported` - no vLLM tool parser and no `tool` role, so they fail SAM's first hard gate (H1) regardless of score:
...
- **Score rank ≠ verdict.** DeepSeek-R1-0528 is #2 by score but `validate-first`, and Gemma 2 would land mid-table on its 52.21 yet is `unsupported` (which is why it sits in the excluded table, not the ranking). Both illustrate why the verdict column, not the number, decides SAM fitness.
